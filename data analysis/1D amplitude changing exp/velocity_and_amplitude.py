...
def compute_velocity_and_peak_accel(
    ...
    base1 = float(np.mean(s1[:n_baseline]))
    base2 = float(np.mean(s2[:n_baseline]))
    s1b = s1 - base1
    s2b = s2 - base2
    ...
    peak_accel_mps2 = (s2v[i2]) / volts_per_mps2
...
if __name__ == '__main__':
    ...
    try:
        results = compute_velocity_and_peak_accel(
            r'.\8_21 main data\20V\scope_32.csv',
            distance=0.018*8,
            sep=',',
            skiprows=2,
            threshold_frac=0.5,
            prominence=0.01,
            volts_per_mps2=0.00102,
            show=True,
            smooth=True,
            smooth_window=401,
            smooth_polyorder=3,
